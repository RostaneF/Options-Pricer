...
public:
    ...
    void generate(int n) {
        ...
        if (!option->isAsianOption()) {
            T = T = option->getExpiry();
            nb_paths += n;
            double ST = 1;
            for (int i = 0; i < n; i++) {
                double z = mt::norm_gen();
            ...
            // Génération du payoff : -----------------------------------------------------------------------------
            for (int i = 0; i < n; i++) { // On souhaite générer n nouveau payoffs que l'on stockera pour estimer le prix et l'affiner avec la loi des grands nombres

                // Génération des norm random : -----------------------------------------------------------------------------
                vector<double> z(m);
                for (int i = 0; i < m; i++) {
                    z[i] = mt::norm_gen();
                }
                // Génération et actualisation de _payoff ------------------------------------------------------------------
                vector<double> St(m);
